...
#'
#' @param folder Character. Path to the project folder containing 
#'   \file{sister_clades.rds}, \file{gap/}, and \file{fast/} subdirectories.
#' @param min_gap_len Integer (bp). Minimum gap length to retain when computing 
#'   flanking-gap statistics (default: \code{500}).
#' @param min_gap_len_for_abs Integer (bp). Minimum flanking-gap length used to 
#'   classify a locus as absent (default: \code{1000}).
#' @param buffered_size Integer vector (bp). Symmetric buffer sizes applied when
#'   recomputing flanking-gap lengths (default: \code{c(100, 250)}).
#' @param min_available_data Numeric in \code{[0,1]}. Minimum fraction of species 
...
#' @param min_clade_size Integer. Clades with fewer than this number of species 
...
#' }
#'
#' @return Invisibly returns \code{NULL}. This function is called for side effects 
#' (file generation).
#'
...
clean_clade_data_fast <- function(folder, min_gap_len = 500, min_gap_len_for_abs = 1000, buffered_size = c(100, 250), min_available_data = 0.1, min_clade_size = 10, chrom = NULL){
